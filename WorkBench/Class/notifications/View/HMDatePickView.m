...
- (void)configuration {
    //时间选择器
    UIView *dateBgView = [[UIView alloc] initWithFrame:CGRectMake(0, self.frame.size.height - 250, [UIScreen mainScreen].bounds.size.width, 200)];
    dateBgView.backgroundColor = [UIColor whiteColor];
    [self addSubview:dateBgView];
    
    //确定按钮
    UIButton *commitBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    commitBtn.frame = CGRectMake(dateBgView.bounds.size.width - 70, 5, 60, 30);
    commitBtn.backgroundColor = RGB(255, 146, 50);
    commitBtn.tag = 1;
    ...
    UIDatePicker *datePicker=[[UIDatePicker alloc]initWithFrame:CGRectMake(0, 40, [UIScreen mainScreen].bounds.size.width, 150)];
