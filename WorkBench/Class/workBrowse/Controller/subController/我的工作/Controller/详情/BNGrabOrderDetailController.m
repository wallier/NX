...
@interface BNGrabOrderDetailController ()<UITableViewDataSource,UITableViewDelegate, UIScrollViewDelegate,UIGestureRecognizerDelegate>
{
    //数据部分
    NSArray *couponArry;
...
@implementation BNGrabOrderDetailController
...
-(void)paint:(NSTimer *)paramTimer
{
    ...
    {
        //保持在线接口
        NSLog(@"_onlineUrl==%@", _onlineUrl);
        [BNNetworkTool initWitUrl:_onlineUrl andParameters:nil andStyle:NO].requestData = ^(id requestDate){
            NSLog(@"_onlineUrl111==%@", requestDate);
        };
    }
...
{
    // 定义一个NSTimer
    self.paintingTimer = [NSTimer scheduledTimerWithTimeInterval:20.0 target:self selector:@selector(paint:) userInfo:nil repeats:YES];
}
...
//    _groupbuyButton.showsTouchWhenHighlighted = YES;  //指定按钮被按下时发光
    [_groupbuyButton setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];//此时未被选中
...
//    _titleLabLeft.text = _titleLabLeftStr;
    }
...
//        }
//        
//        _titleLabLeft.text = [NSString stringWithFormat:@"  %d/%@户", _cplus,requestData[@"SIZE"]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    {
    _hud = [MBProgressHUD showMessage:@"加载中..." toView:self.view.window];
    }
...
//    NSLog(@"strUrl====%@",strUrl);
    ...
    if([[LoginModel shareLoginModel].ORG_MANAGER_TYPE isEqualToString:@"HX"]||[[LoginModel shareLoginModel].ORG_MANAGER_TYPE isEqualToString:@"WG"]){
        ...
        if([cell.model.ORDER_STATE isEqualToString:@"0"]){
            ...
            = ^(id request){
                //进入工单
                //[[request valueForKey:@"MSGCODE"] isEqualToString:@"0"]
